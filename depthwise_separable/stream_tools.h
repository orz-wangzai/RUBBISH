...
//���������ͦ��Ҫ�ģ��ÿ�һ�� ���InWidth > Outwidth NumInwords ����In��size ��֮��Ȼ
...
#pragma HLS PIPELINE II = 1
        ...
        const unsigned int inPerOut = OutWidth / InWidth; // equals to Outchannel/PE?
        cout << " the inPerOut is " << inPerOut << endl;
        const unsigned int totalIters = NumInWords * numReps; //the size of the output matrix
        cout << " the size of the matrix  is " << totalIters << endl;
        unsigned int i = 0;
        ap_uint<OutWidth> eo ;
        cout << "????" << InWidth << endl;
        cout << "????" << OutWidth << endl;
...
//#pragma HLS PIPELINE II = 1
        	...
        	for (i = 0; i < inPerOut; i++)
            {
        		...
        		//cout << "i " << i << " ei: " << bitset<InWidth>(ei) << endl;
        		//count++;
        		//cout << "i " << i << " before eo: " << bitset<OutWidth>(eo) << endl;

        		//eo(71,64) = 11111111;
        		eo = eo >> InWidth;
                eo(OutWidth - 1, OutWidth-InWidth) = ei;

                //cout << "i " << i << " eo: " << bitset<OutWidth>(eo) << endl;
